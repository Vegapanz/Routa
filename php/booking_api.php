...
// Prevent drivers and admins from using user booking API
...
if (isset($_SESSION['is_admin']) && $_SESSION['is_admin'] === true) {
    echo json_encode(['success' => false, 'message' => 'Admins cannot access user booking functions']);
    exit;
}
...
function createBooking($pdo, $userId) {
    ...
    // Create notification for admins about new booking request
...
function rateDriver($pdo, $userId) {
    ...
    // Verify booking - allow re-rating
    $stmt = $pdo->prepare("
        SELECT * FROM ride_history 
    ...
    if (!$booking) {
        error_log("Booking not found or not completed");
