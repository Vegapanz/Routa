...
if (!isset($_SESSION['user_id']) || $_SESSION['is_driver'] !== true) {
    echo json_encode(['success' => false, 'message' => 'Driver authentication required']);
    exit;
}
...
function rejectRide($pdo, $driverId) {
    ...
    $stmt = $pdo->prepare("
        UPDATE ride_history 
        SET driver_id = NULL, status = 'searching', updated_at = NOW() 
        WHERE id = ?
    ");
    $stmt->execute([$rideId]);
    ...
    $nearbyDrivers = findNearbyDrivers($pdo, $ride['pickup_lat'], $ride['pickup_lng'], 5, $driverId);
    if (!empty($nearbyDrivers)) {
        $newDriver = $nearbyDrivers[0];
        $stmt = $pdo->prepare("
            UPDATE ride_history 
            SET driver_id = ?, status = 'driver_found', updated_at = NOW() 
            WHERE id = ?
        ");
        $stmt->execute([$newDriver['id'], $rideId]);
        
        createNotification($pdo, $rideId, $newDriver['id'], 'driver', 'new_ride', 
            "New ride request from {$ride['pickup_location']}");
    }
