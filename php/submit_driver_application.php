...
try {
    ...
    // Check for duplicate email in driver_applications
    $checkStmt = $pdo->prepare("SELECT id FROM driver_applications WHERE email = ?");
    $checkStmt->execute([$email]);
    if ($checkStmt->fetch()) {
        throw new Exception('An application with this email address already exists. Please use a different email or contact support if you need assistance.');
    }

    // Check for duplicate email in approved drivers
    $checkStmt = $pdo->prepare("SELECT id FROM tricycle_drivers WHERE email = ?");
    $checkStmt->execute([$email]);
    if ($checkStmt->fetch()) {
        throw new Exception('This email address is already registered as a driver. Please login to your driver account or contact support.');
    }

    // Check for duplicate phone number in driver_applications
    $checkStmt = $pdo->prepare("SELECT id FROM driver_applications WHERE phone = ?");
    $checkStmt->execute([$phone]);
    if ($checkStmt->fetch()) {
        throw new Exception('An application with this phone number already exists. Please use a different phone number or contact support.');
    }

    // Check for duplicate phone number in approved drivers
    $checkStmt = $pdo->prepare("SELECT id FROM tricycle_drivers WHERE phone = ?");
    $checkStmt->execute([$phone]);
    if ($checkStmt->fetch()) {
        throw new Exception('This phone number is already registered as a driver. Please login to your driver account or contact support.');
    }

    // Check for duplicate license number in driver_applications
    if (!empty($licenseNumber)) {
        $checkStmt = $pdo->prepare("SELECT id FROM driver_applications WHERE license_number = ?");
        $checkStmt->execute([$licenseNumber]);
        if ($checkStmt->fetch()) {
            throw new Exception('An application with this license number already exists. Each license can only be used for one application.');
        }

        // Check for duplicate license number in approved drivers
        $checkStmt = $pdo->prepare("SELECT id FROM tricycle_drivers WHERE license_number = ?");
        $checkStmt->execute([$licenseNumber]);
        if ($checkStmt->fetch()) {
            throw new Exception('This license number is already registered as a driver. If you believe this is an error, please contact support.');
        }
    }

    // Check for duplicate plate number in driver_applications
    if (!empty($plateNumber)) {
        $checkStmt = $pdo->prepare("SELECT id FROM driver_applications WHERE plate_number = ?");
        $checkStmt->execute([$plateNumber]);
        if ($checkStmt->fetch()) {
            throw new Exception('An application with this plate number already exists. Each vehicle can only be registered once.');
        }

        // Check for duplicate plate number in approved drivers
        $checkStmt = $pdo->prepare("SELECT id FROM tricycle_drivers WHERE plate_number = ?");
        $checkStmt->execute([$plateNumber]);
        if ($checkStmt->fetch()) {
            throw new Exception('This plate number is already registered. Each vehicle can only be registered to one driver.');
        }
    }
    ...
    // Send thank you email to the driver
    try {
        $emailSent = sendDriverApplicationThankYou($email, $firstName, $lastName, $applicationId);
        
        if ($emailSent) {
            error_log("Driver application thank you email sent to: {$email}");
        } else {
            error_log("Failed to send driver application thank you email to: {$email}");
        }
    } catch (Exception $emailError) {
        // Log email error but don't fail the application submission
        error_log("Error sending driver application email: " . $emailError->getMessage());
    }

    // Send success response
    echo json_encode([
        'success' => true,
        'message' => 'Application submitted successfully! We will review your application and get back to you within 2-3 business days. A confirmation email has been sent to your email address.',
